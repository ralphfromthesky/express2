...
import { body, validationResult, query, checkSchema, matchedData } from "express-validator"; // this one for validation of inputs " npm install express-validator"
...
mongoose.connect('mongodb://localhost/express_tutorial').then(() => console.log('connected to the database')).catch((err) => console.log(`there is err: ${err}`))
...
app.post("/api/user", checkSchema(createUserValidationSchema), async (request, response) => {
